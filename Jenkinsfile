        ...
        stage('Publish') {
            ...
            steps{
                container('docker') {
                    ...
                    sh 'docker build -t ${docker_app} -t ${docker_app}:$BUILD_NUMBER -f ~/chatbot/app/Dockerfile .'
                    sh 'docker build -t ${docker_app2} -t ${docker_app2}:$BUILD_NUMBER -f ~/chatbot/app/actions/Dockerfile .'
